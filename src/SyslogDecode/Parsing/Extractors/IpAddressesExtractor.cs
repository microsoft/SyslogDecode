...
namespace SyslogDecode.Parsing
{
    ...
    public class IpAddressesExtractor: IValuesExtractor
    {
        ...
        public static IList<NameValuePair> ExtractIpAddresses(IList<NameValuePair> fromValuesOf)
        {
            ...
            var ipv4s = fromValuesOf
                                .Where(p => IpAddressesExtractor.IsIpv4Address(p.Value))
                                .Select(p => p.Value)
                                .Distinct()
                                .Select(ip => new NameValuePair() { Name = "IPv4", Value = ip })
                                .ToList();
            var ipv6s = fromValuesOf
                          .Where(p => IpAddressesExtractor.IsIpv6Address(p.Value))
                          .Select(p => p.Value)
                          .Distinct()
                          .Select(ip => new NameValuePair() { Name = "IPv6", Value = ip})
                          .ToList();
